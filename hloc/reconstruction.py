        ...
        if ref_image.name in img_names:
            ...
            db.execute(f'''UPDATE images
                        SET prior_qw = {ref_image.qvec[0]},
                            ...
                            prior_qy = {ref_image.qvec[2]},
                            prior_qz = {ref_image.qvec[3]},
                            prior_tx = {ref_image.tvec[0]},
                            prior_ty = {ref_image.tvec[1]},
                            prior_tz = {ref_image.tvec[2]}
                        WHERE
                            name='{ref_image.name}'
                       ''')
    db.commit()
...
def main(sfm_dir, image_dir, pairs, features, matches,
         ...
         image_list: Optional[List[str]] = None, intrinsics_path=None, prior_pose=None):
